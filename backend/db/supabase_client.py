...
_RETRYABLE_ERRORS = ("Server disconnected", "RemoteProtocolError", "ReadTimeout")
...
class SupabaseDB:
    ...
    _client: Optional[Client] = None
    _lock = threading.Lock()  # FIX: prevent race condition on concurrent first requests

    @classmethod
    def get_client(cls) -> Client:
        """Get or create Supabase client (singleton pattern, thread-safe)"""
        if cls._client is None:
            with cls._lock:
    ...
    def select_one(
        ...
        """Select a single record by ID"""
        response = cls._safe_execute(
            lambda: cls.get_client().table(table).select(columns).eq("id", id).limit(1).execute()
            # FIX: added .limit(1) — no need to fetch more rows than we'll ever use
        )
        return response.data[0] if response.data else None
    ...
    def select_in(
        ...
        if not values:
            return []
        response = cls._safe_execute(
            lambda: cls.get_client().table(table).select(columns).in_(column, values).execute()
        )
        return response.data or []
    ...
    def _safe_execute(cls, func, max_retries: int = 3) -> Any:
        """
        Wraps database execution with retry logic for transient connection errors.
        Uses exponential back-off and forces client re-initialisation on each retry.
        """
        for attempt in range(1, max_retries + 1):
            try:
                ...
                e_str = str(e)
                is_retryable = any(sig in e_str for sig in _RETRYABLE_ERRORS)
                # FIX: also check exception *type* name for ReadTimeout
                is_retryable = is_retryable or "ReadTimeout" in type(e).__name__

                if not is_retryable or attempt == max_retries:
                    if is_retryable:
                        logger.error(
                            f"Supabase connection retry failed after {max_retries} attempts: {e}"
                        )
                    raise

                # Exponential back-off: 0.5s, 1.0s, 1.5s …
                # FIX: was doing 0.5 * attempt AFTER incrementing attempt, making
                #      the first sleep 1.0s instead of 0.5s. Now explicit and clear.
                delay = 0.5 * attempt
                logger.warning(
                    f"Supabase connection issue ({type(e).__name__}). "
                ...
                time.sleep(delay)
                cls._client = None  # force fresh connection on next attempt
    ...
    def broadcast(cls, channel: str, event: str, payload: Dict[str, Any]) -> bool:
        """
        Broadcast a Realtime message directly (ephemeral) via the REST API.
        Uses a short-lived httpx client to avoid stale connection issues.
        """
        import httpx  # local import — only used here, no need to load at module level
        ...
        try:
            ...
            if response.status_code != 200:
                logger.warning(
                    f"Realtime broadcast HTTP error: {response.status_code} - {response.text}"
                )
            return response.status_code == 200
        except Exception as e:
            logger.error(f"Realtime broadcast exception: {e}")
